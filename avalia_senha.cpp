...
void montaCombinacoesDeTestes(CombinacoesArquivos combinacoes[]) {
  char nomes_arquivos_de_dados[N_ARQUIVOS_DADOS][LENGHT_STRINGS] = {
      "data/500_ordenado.csv",    "data/500_desordenado.csv",
      "data/10mil_ordenado.csv",  "data/10mil_desordenado.csv",
      "data/100mil_ordenado.csv", "data/100mil_desordenado.csv"};

  char arquivos_de_teste_500[N_ARQUIVOS_TESTES_500][LENGHT_STRINGS] = {
      "data/10u_base500_1e.csv", "data/10u_base500_5e.csv",
      "data/100u_base500_10e.csv", "data/100u_base500_50e.csv"};

  char arquivos_de_teste_10mil[N_ARQUIVOS_TESTES][LENGHT_STRINGS] = {
      "data/10u_base10mil_1e.csv",     "data/10u_base10mil_5e.csv",
      "data/100u_base10mil_10e.csv",   "data/100u_base10mil_50e.csv",
      "data/1000u_base10mil_100e.csv", "data/1000u_base10mil_500e.csv"};

  char arquivos_de_teste_100mil[N_ARQUIVOS_TESTES][LENGHT_STRINGS] = {
      "data/10u_base100mil_1e.csv",     "data/10u_base100mil_5e.csv",
      "data/100u_base100mil_10e.csv",   "data/100u_base100mil_50e.csv",
      "data/1000u_base100mil_100e.csv", "data/1000u_base100mil_500e.csv"};
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < N_ARQUIVOS_TESTES_500; j++) {
      ...
      // combinacoes[c].arquivo_de_dados, combinacoes[c].arquivo_de_teste);
      c++;
    }
  }
...
void leDadosDoArquivoEManipulaArvore(char nome_arq_entrada[],
  ...
  else  // Se conseguiu abrir o arquivo para leitura
  {
    ...
    while (!feof(ptr_arquivo)) {
      if ((id_lido = carregaSenhaTXT(ptr_arquivo,
                                     senha_lida)) !=
          -1)  // Se conseguiu ler o usuario e a senha e
      {
...
void testa_arvores(char *arquivo_carrega, char *arquivo_teste) {
  ...
  leDadosDoArquivoEManipulaArvore(arquivo_carrega, &ABP, ARVORE_ABP,
                                  INSERE);  // Insere
  printf("Buscando na ABP\n");
  leDadosDoArquivoEManipulaArvore(arquivo_teste, &ABP, ARVORE_ABP,
                                  CONSULTA);  // Consulta

  printf("----------------\n");
  // AVL
  printf("Inserindo na AVL\n");
  leDadosDoArquivoEManipulaArvore(arquivo_carrega, &AVL, ARVORE_AVL,
                                  INSERE);  // Insere
  printf("Buscando na AVL\n");
  leDadosDoArquivoEManipulaArvore(arquivo_teste, &AVL, ARVORE_AVL,
                                  CONSULTA);  // Consulta
  printf(
      "========================================================================"
      "====================\n\n");
